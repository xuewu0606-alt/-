...
- 乖离率 < 2%：最佳买点区间
- 乖离率 2-5%：可小仓介入
- 乖离率 > 5%：严禁当下追入，但不得只给一句"观望"——必须写明可执行的等待条件
  （如"回踩 MA5 约 X 元缩量企稳可介入"），见基线4的两通道买点体系

### 2. 趋势交易（顺势而为）
- **多头排列必须条件**：MA5 > MA10 > MA20
- 只做多头排列的股票，空头排列坚决不碰
- **例外通道**：空头排列下，若“放量站上MA20 + 重大利好催化”同时成立，可判定为“震荡（反转观察）”，允许轻仓试仓（不超过2成仓）并设明确失效止损（收盘跌回MA20下方离场）；缺任一条件仍维持观望
- 均线发散上行优于均线粘合
...
### 4. 买点体系（左侧回踩 + 右侧突破，两条通道并行）
- **回踩买点（左侧，优先）**：缩量回踩 MA5 获得支撑最佳；次优回踩 MA10
- **突破试仓买点（右侧确认）**：当日或前一交易日放量（量比≥1.5）收盘站上 MA20，
  且乖离 MA5 ≤5%、偏离 MA20 ≤10% → 应给出**轻仓试仓**方案（不超过2成仓），
  止损设"收盘跌回 MA20 离场"；此后缩量回踩 MA5/MA10 企稳再加仓至目标仓位
- **严禁把"等待完美回踩"当成唯一入口**：下跌时说"空头不碰"、启动时说"不追高等回踩"、
  强势股又常不回踩——三连否决等于任何价格都无法入场，这不是风控而是决策瘫痪。
  趋势启动初期（刚放量站上 MA20、涨幅未失控）必须在两个通道中给出可执行入场方案
- **观望情况**：跌破 MA20；或乖离 MA5 >5% 且不满足强势趋势豁免（此时给出"若回踩
  至 X 元附近企稳可介入"的具体等待条件，而非无条件观望）
